...
win.withdraw()
#寻找是否有本地更新包
if not os.path.exists('updater/offline'):
    os.mkdir('updater/offline')
if os.path.exists('updater/offline/app-update.zip'):
    asks = messagebox.askyesnocancel('Seewo Utils离线更新','检测到Seewo Utils的离线更新包，是否进行更新\n建议从官方Github下载更新包，对第三方更新包引起的程序出错概不负责\n单击“是”以进行更新，单击“否”以在线搜索更新，单击“取消”以取消更新')
    if asks == True:
        os.system('taskkill /t /f /im seewoutils.exe')
        #删除源文件
        ...
        filen.close()
        #启动程序
        os.system('start seewoutils.exe')
        sys.exit()
    elif asks == None:
        sys.exit()
#获取github ip
...
#获取最新版本信息
try:
    info = requests.get('https://api.github.com/repos/alexliu07/SeewoUtils/releases').text
...
if current_version < latest_version:
    ...
    for i in latest_version_detail['assets']:
        if i['name'] == 'app-update.zip':
            update_size = i['size']
    #询问是否更新
    asks = messagebox.askokcancel('Seewo Utils更新','检测到Seewo Utils有更新，是否进行更新？\n新版本：{}    当前版本：{}\n更新内容：\n{}'.format(latest_version,current_version,latest_version_detail['body']))
    if asks:
        #检测文件夹是否存在
        ...
        if not os.path.exists('updater/tmp/tmp.zip'):
            print('正在下载更新...')
            try:
                filed = requests.get('https://'+githuburl+'/alexliu07/SeewoUtils/releases/download/'+str(latest_version)+'/app-update.zip',verify=False)
    ...
    messagebox.showinfo('未检测到更新','Seewo Utils为最新版本！')